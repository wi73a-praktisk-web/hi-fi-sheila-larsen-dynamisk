            ...
            fetch(request)
                .then(response => {
                    ...
                    if (response.status == 200) {
                        ...
                        throw new Error(`Bruger blev ikke opdateret: ${response.statusText}`)
                    }
                }).catch(err => {
                    console.log(err);
                });
...
fetch('http://localhost:1337/opret_bruger')
 ...
 <div class="col-xs-2"> 
 <p class="opret_bruger_p">${item.id}</p>                   
 </div> 

 <div class="col-xs-2">                 
 <p class="opret_bruger_p">${item.username}</p>
 </div>
 ...
 <div class="col-xs-3">
 <button class="btn opret_bruger_btn_ret" data-brugerid="${item.id}">RET</button>
 </div> 

 <div class="col-xs-3">
  <button class="btn opret_bruger_btn_slet" data-brugerid="${item.id}">SLET</button>
 </div> 
 </div>             
 `
        })
        let sletKnapper = document.querySelectorAll("button.opret_bruger_btn_slet");
        sletKnapper.forEach((button) => {
            button.addEventListener("click", sletbruger)
        })

        let retKnapper = document.querySelectorAll("button.opret_bruger_btn_ret");
